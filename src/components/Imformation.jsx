...
function Imformation() {
  ...
  // This is the correct ref for managing carousel items in each grid cell.
  const individualCarouselItemRefs = useRef(
    Array(9).fill(null).map(() => Array(carouselItems.length).fill(null))
  );

  // This is the correct function to add refs for individual carousel items.
  ...
  // REMOVE THE OLD 'carouselItemRefs' AND 'addToCarouselRefs' HERE.
  // const carouselItemRefs = useRef([]); // DELETE THIS LINE
  // const addToCarouselRefs = (el, itemIndex) => { // DELETE THIS FUNCTION
  //   if (el && carouselItemRefs.current[itemIndex] !== el) {
  //     carouselItemRefs.current[itemIndex] = el;
  //   }
  // };
